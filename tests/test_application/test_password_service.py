...
def test_get_password_hash_given_invalid_email_raises_invalid_request_exception(password_service):
    with pytest.raises(InvalidRequestException) as excinfo:
...
def test_get_password_hash_given_invalid_password_raises_invalid_password_exception(password_service):
...
def test_get_password_hash_given_valid_password_raises_password_service_exception_on_no_update(password_service_empty_repo):
    with pytest.raises(PasswordServiceException):
...
def test_get_password_hash_given_valid_password_returns_none(password_service):
    assert password_service.validate_credential('user_id', 'password') is None
